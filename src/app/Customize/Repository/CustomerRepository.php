...
class CustomerRepository extends AbstractRepository
{
     ...
     */
    public function getProvisionalCustomerByEmail($email)
    {
    ...
    /**
     * 仮会員をリセットキー、またはリセットキーとメールアドレスで検索する.
     *
     * @param $resetKey
     * @param $email
     *
     * @return Customer|null 見つからない場合はnullを返す.
     */
    public function getProvisionalCustomerByResetKey($resetKey, $email = null)
    {
        $qb = $this->createQueryBuilder('c')
     ...
     * @return Customer|null 見つからない場合はnullを返す.
     */
    public function getRegularCustomerBySecretKey($secret_key)
    {
        return $this->findOneBy([
     ...
     * 会員一覧用の検索にカート内商品数を付加
     */
    public function getCartProductsBySearchData($searchData)
    {
        $qb = parent::getCartProductsBySearchData($searchData);

        // CartItem を JOIN
        $qb->addSelect('COUNT(ci.id) AS cart_item_count')
            ->leftJoin('c.Carts', 'cart')
            ->leftJoin('cart.CartItems', 'ci')
            ->groupBy('c.id');
