 ...
 */
class OrderInvoiceService extends Fpdi
{
    /** @var OrderRepository */
    ...
    public const DEFAULT_PDF_FILE_NAME = 'invoice.pdf';
    ...
    public function __construct(EccubeConfig $eccubeConfig, OrderRepository $orderRepository, ShippingRepository $shippingRepository, TaxRuleService $taxRuleService, BaseInfoRepository $baseInfoRepository, EccubeExtension $eccubeExtension, TaxExtension $taxExtension)
    {
        ...
        $this->setFooterFont([self::FONT_GOTHIC, '', 8]);
    }
    ...
    {
        // 発行日の設定
        $this->issueDate = '発行日　'.$formData['issue_date']->format('Y/m/d');
        // ダウンロードファイル名の初期化
        ...
        foreach ($ids as $id) {
            ...
            if ($Order->isMultiple()) {
                ...
                $userPath = $this->eccubeConfig->get('eccube_theme_admin_dir').'/assets/pdf/invoice_multiple.pdf';
            } else {
                $userPath = $this->eccubeConfig->get('eccube_theme_admin_dir').'/assets/pdf/invoice.pdf';
            }
            ...
            // タイトルを描画する
            $this->renderTitle(trans('admin.order.output_invoice_title')); //$formData['title']);

            // 店舗情報を描画する
            $this->renderShopData();

            // 注文情報を描画する
            $this->renderOrderData($Shipping, $formData['address_kind']);
    ...
    public function getPdfFileName()
    {
        ...
        if ($this->PageNo() == 1) {
            $this->downloadFileName = 'seikyusyo-No'.$this->lastOrderId.'.pdf';
        }
    ...
    public function Footer()
    {
        ...
        $this->lfText(166, 45, $this->issueDate, 8, 'R');
        $this->lfText(166, 41, 'No. 　　　　 '.str_pad($this->lastOrderId, 6, 0, STR_PAD_LEFT), 8, 'R');
    }
    ...
    protected function renderShopData()
    {
        ...
        // 会社名
        $this->lfText($xPosition, 58, $this->baseInfoRepository->getCompanyName(), 10, 'B');

        //郵便番号
        $this->lfText($xPosition, 64, "\u{3012}". ' ' . mb_substr($this->baseInfoRepository->getPostalCode(), 0, 3) . ' - ' . mb_substr($this->baseInfoRepository->getPostalCode(), 3, 4), $fontText);


        // 都道府県+所在地
        $text = $this->baseInfoRepository->getPref().$this->baseInfoRepository->getAddr01();
        $this->lfText($xPosition, 69, $text, $fontText);
        $this->lfText($xPosition, 74, $this->baseInfoRepository->getAddr02(), $fontText);

        // 電話番号
        $text = 'TEL: '.$this->baseInfoRepository->getPhoneNumber();
        $this->lfText($xPosition, 79, $text, $fontText); // TEL・FAX
        ...
        */
        ...
        if (!empty($this->baseInfoRepository->getInvoiceRegistrationNumber())) {
            ...
            $this->lfText($xPosition, 84, $text, $fontText);
        }
    ...
    {
        $this->lfText(20, 70, trans('admin.order.output_invoice_message'), 9); // メッセージ1 $formData['message1']
        //$this->lfText(20, 74, $formData['message2'], 8); // メッセージ2
        //$this->lfText(20, 78, $formData['message3'], 8); // メッセージ3
    }
     ...
     * @param array $formData
     */
    protected function renderBankData(array $formData)
    {
        // フォント情報のバックアップ
        $this->backupFont();
        ...
        $this->MultiCell(5, 6, '', 0, 2, 'L', 0, '');
        $this->MultiCell(168, 6, '振込先', 'B', 2, 'L', 0, '');

        $this->SetFont(self::FONT_GOTHIC, '', 9);

        $this->Ln();
        $this->Cell(0, 1, '', 0, 1, 'C', 0, '');
        $this->MultiCell(5, 6, '', '', 2, 'L', 0, '');
        $this->MultiCell(0, 4,  trans('admin.order.output_invoice_bank_account'), '', 2, 'L', 0, '');
    ...
    {   
        ...
        $this->SetFont(self::FONT_GOTHIC, 'B', 10);
        $this->MultiCell(5, 6, '', '', 2, 'L', 0, '');
        $this->MultiCell(168, 6, '備考', 'B', 2, 'L', 0, '');
     ...
     * @param int $address_kind
     */
    protected function renderOrderData(Shipping $Shipping, $address_kind)
    {
        // 基準座標を設定する
        $this->setBasePosition();
        ...
        if ($address_kind == 1) {
            // 学校向け(学校名)
            $text = $Shipping->getCompanyName().'　御中';;
            $this->lfText(20, 58, $text, 16);
        } else {
            // 個人向け(学校名＋保護者名)
            $text = $Shipping->getCompanyName().'';;
            $this->lfText(20, 54, $text, 8);
            $text = $Shipping->getName01().'　'.$Shipping->getName02().'　保護者様';
            $this->lfText(20, 58, $text, 16);
        }
        ...
        $this->lfText(40, 121, $orderDate, 10);
        ...
        // 総合計金額
        if (!$Order->isMultiple()) {
            $this->SetFont(self::FONT_GOTHIC, 'B', 15);
            //$paymentTotalText = $this->eccubeExtension->getPriceFilter($Order->getPaymentTotal());
            $paymentTotalText = number_format($Order->getPaymentTotal()).' 円';
            $this->lfText(78, 102, '(税込)', 9);

            $this->setBasePosition(120, 95.5);
            $this->Cell(5, 7, '', 0, 0, '', 0, '');
            $this->Cell(59, 8, $paymentTotalText, 0, 2, 'R', 0, '');
    ...
    protected function renderOrderDetailData(Shipping $Shipping)
    {
        ...
        $this->backupFont();
        $this->SetLineWidth(.3);
        $this->SetFont(self::FONT_GOTHIC, '', 6);
        ...
        foreach ($Order->getTotalByTaxRate() as $rate => $total) {
            ...
            $message .= ' うち消費税: '.$this->eccubeExtension->getPriceFilter($Order->getTaxByTaxRate()[$rate]).')'.PHP_EOL;
        }
    ...
    protected function setFancyTable($header, $data, $w)
    {
        ...
        $this->SetLineWidth(.3);
        $this->SetFont(self::FONT_GOTHIC, 'B', 8);
        $this->SetFont('', 'B');
