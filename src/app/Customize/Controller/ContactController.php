...
class ContactController extends AbstractController
{
     ...
     * 弁済情報通知用メールアドレス確認画面.
     *
     * @Route("/contact/repay", name="contact_repay", methods={"GET", "POST"})
    ...
    public function repaly(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            switch ($request->get('mode')) {
                case 'confirm':
                    return $this->render('Contact/repay_confirm.twig', [
                        'form' => $form->createView(),
                        'Page' => $this->pageRepository->getPageByRoute('contact_confirm'),
                        'is_mobile' => $is_mobile,
                    ]);
    ...
    /**
     * 弁済情報通知用メールアドレス確認完了画面.
     *
     * @Route("/contact/repay_complete", name="contact_repay_complete", methods={"GET"})
     * @Template("Contact/repay_complete.twig")
     */
    public function repay_complete()
    {
        // スマホ表示フラグ
        $detect = new MobileDetect;
        $is_mobile = $detect->isMobile();
