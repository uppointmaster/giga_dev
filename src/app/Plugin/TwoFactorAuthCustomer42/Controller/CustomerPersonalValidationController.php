...
use Plugin\TwoFactorAuthCustomer42\Form\Type\TwoFactorAuthMailTypeCustomer;
...
class CustomerPersonalValidationController extends AbstractController
{
     ...
     * ２段階認証コード送信画面.
     *
     * @Route("/two_factor_auth/mail/send/{secret_key}", name="plg_customer_2fa_mail_auth_send_onetime", requirements={"secret_key" = "^[a-zA-Z0-9]+$"}, methods={"GET", "POST"})
     * @Template("TwoFactorAuthCustomer42/Resource/template/default/mail_auth/send.twig")
     *
     * @param Request $request
     * @param $secret_key
     *
     ...
     * @throws TwilioException
     */
    public function mailAuthSendOneTime(Request $request, $secret_key)
    {
        $Customer = $this->getUser();
        if ($this->isGranted('ROLE_USER') && $Customer->isDeviceAuthed()) {
            // 認証済みならばカートへ
        ...
        $error = null;
        /** @var Customer $Customer */
        $Customer = $this->customerRepository->getRegularCustomerBySecretKey($secret_key);
        if ($Customer === null) {
            throw $this->createNotFoundException();
        }
        $builder = $this->formFactory->createBuilder(TwoFactorAuthMailTypeCustomer::class);
        // 入力フォーム生成
        $form = $builder->getForm();
        if ('POST' === $request->getMethod() or 'GET' === $request->getMethod()) {
            $form->handleRequest($request);
            if ($form->isSubmitted() or 'GET' === $request->getMethod()){ // && $form->isValid()) {
                // メールでスロットリングをかける
                $limiter = $this->deviceAuthRequestEmailLimiter->create($Customer->getEmail());
                if (!$limiter->consume()->isAccepted()) {
                    throw new TooManyRequestsHttpException();
                ...
                // 登録メールアドレスへ認証コードを送信
                $this->sendMailToken($Customer, $email);
                // 認証コードをセッションへ一時格納
                $this->session->set(
                    CustomerTwoFactorAuthService::SESSION_AUTHED_PHONE_NUMBER,
                    $email
                ...
                return $this->redirectToRoute(
                    'plg_customer_2fa_mail_auth_input_onetime',
                    ['secret_key' => $secret_key]
                );
            } else {
                $error = trans('front.2fa.sms.send.failure_message');
     ...
     * メール認証ワンタイムトークン入力画面.
     *
     * @Route("/two_factor_auth/mail_auth/input/{secret_key}", name="plg_customer_2fa_mail_auth_input_onetime", requirements={"secret_key" = "^[a-zA-Z0-9]+$"}, methods={"GET", "POST"})
     * @Template("TwoFactorAuthCustomer42/Resource/template/default/mail_auth/input.twig")
     *
     * @param Request $request
     * @param $secret_key
     *
     * @return array|RedirectResponse
     */
    public function mailAuthInputOneTime(Request $request, $secret_key)
    {
        $Customer = $this->getUser();
        if ($this->isGranted('ROLE_USER') && $Customer->isDeviceAuthed()) {
            // 認証済みならばカートへ
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if (!$this->checkDeviceToken($Customer, $token)) {
                    ...
                    // リダイレクト
                    $flg_entry_complete = $this->session->get(EccubeEvents::FRONT_FORGOT_ENTRY_COMPLETE);
                    if ($flg_entry_complete) {
                        return $this->redirectToRoute('cart');
                    } else {
                        return $this->redirectToRoute('forgot_entry_detail');
                    }
                }
            } else {
                $error = trans('front.2fa.onetime.invalid_message__reinput');
     ...
     * @throws \Exception
     */
    private function sendMailToken(Customer $Customer, string $emailAddress)
    {
        // ワンタイムトークン生成・保存
        $token = $this->customerTwoFactorAuthService->generateOneTimeTokenValue();
        ...
        // メール送信
        return $this->mailService->sendForgotAuthNotificationMail($Customer, $token);
    }
}
